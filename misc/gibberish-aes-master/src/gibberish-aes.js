...
var GibberishAES = (function(){
    ...
    randArr = function(num) {
        var result = [], i;
        for (i = 0; i < num; i++) {
            result = result.concat(Math.floor(Math.random() * 256));//random salt
			//result = result.concat(i);//test non random salt (for debugging data)
        }
        return result;
    },
        ...
        encode = function(b, withBreaks) {
            ...
            for (i = 0; i < flatArr.length; i = i + 3) {
				if (flatArr[i + 1] == undefined) {
					console.log("undefined = "+chars[((flatArr[i] & 3) << 4) | (flatArr[i + 1] >> 4)]);
				} else {
					console.log("defined = "+(i+1));
				}
